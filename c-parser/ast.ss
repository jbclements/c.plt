...
(define (src-start src)
  (make-position (src-start-offset src)
                 (src-start-line src)
                 (src-start-col src)))

(define (src-end src)
  (make-position (src-end-offset src)
                 (src-end-line src)
                 (src-end-col src)))

(define (build-src start end path)
  (make-src (position-offset start)
            (position-line start)
            (position-col start)
            (position-offset end)
            (position-line end)
...
;       (values types (make-decl:declarator src id (apply-type-context context base) init))])))

;; declarator-context * type -> complete-declarator
(define (apply-declarator-context declarator base)
  (match declarator
    [(struct decl:declarator (src id context init))
     (make-decl:declarator src id (apply-type-context context base) init)]))
...
(define (apply-member-declarator-context declarator base)
  (match declarator
    [(struct decl:member-declarator (src id context init bit-size))
     (make-decl:member-declarator src id (apply-type-context context base) init bit-size)]))
